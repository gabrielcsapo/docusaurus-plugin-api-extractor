...
it('replaces <b> around "Extends" heritage types', () => {
  const out = remark().use(repairHeritageTypes).processSync('<b>Extends:</b> [Bar](./bar.md)').contents;
  expect(out).toMatchSnapshot();
});

it('replaces <b> around "References" heritage types', () => {
  const out = remark().use(repairHeritageTypes).processSync('<b>References:</b> [Bar](./bar.md)').contents;
  expect(out).toMatchSnapshot();
});
